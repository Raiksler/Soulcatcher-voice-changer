...
def voice_to_text(file, language):                                                # Конвертация аудиосообщения в формат wav и его распознавание.
    try:                                                                # Через трай проверяем, что голосовое сообщение содержит распознаваемый текст.
        ...
        return recognizer.recognize_google(audio, language=language)         # Непосредственно распознавание. Выбираем API, файл и язык.
